...
JNIEXPORT jfloat JNICALL Java_de_maxhenkel_rnnoise4j_Denoiser_denoiseInPlace0(
    ...
    jshort *input_pcm_buffer = calloc(frame_size, sizeof(jshort));
    float *input_buffer = calloc(frame_size, sizeof(float));
    float *output_buffer = calloc(frame_size, sizeof(float));
    jshort *output_pcm_buffer = calloc(frame_size, sizeof(jshort));
    ...
    for (int i = 0; i < frame_count; i++) {
        (*env)->GetShortArrayRegion(env, input, i * frame_size, frame_size, input_pcm_buffer);
        for (int frame_index = 0; frame_index < frame_size; frame_index++) {
            input_buffer[frame_index] = (float) input_pcm_buffer[frame_index];
        }
